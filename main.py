...
def load_plugins(plugin_name):
    path = Path(f"AUTHOR/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"AUTHOR.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["AUTHOR.modules." + plugin_name] = load
    print("ᴀᴜᴛʜᴏʀ ʜᴀꜱ ɪᴍᴘᴏʀᴛᴇᴅ" + plugin_name)


files = glob.glob("AUTHOR/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
...
print("\ᴀᴜᴛʜᴏʀ ɪꜱ ᴅᴇᴘʟᴏʏᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ 🥀")
